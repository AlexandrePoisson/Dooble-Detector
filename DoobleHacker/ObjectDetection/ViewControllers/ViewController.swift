...
extension ViewController: CameraFeedManagerDelegate {
  ...
  @objc  func runModel(onPixelBuffer pixelBuffer: CVPixelBuffer) {
    ...
    let dooblePic = displayResult.dooblePic
    var infArray: [Inference] = []
    
    for inf in displayResult.inferences
    {
        if inf.className == dooblePic
        {
            infArray.append(inf)
        }
    }
    let filterResult = Result(inferenceTime: displayResult.inferenceTime, inferences: infArray, dooblePic: dooblePic)
    ...
    DispatchQueue.main.async {
      ...
      self.inferenceViewController?.inferenceTime = inferenceTime
        ...
        var dooblePic: String = "ini"
        if let resultDooblePic = self.result?.dooblePic {
            dooblePic = resultDooblePic
        }
        self.inferenceViewController?.dooblePic = dooblePic

      self.inferenceViewController?.tableView.reloadData()

      // Draws the bounding boxes and displays class names and confidence scores.
        let drawOnlyDooble = self.inferenceViewController!.filterSwitch.isOn

        
        if drawOnlyDooble {
          self.drawAfterPerformingCalculations(onInferences: filterResult.inferences, withImageSize: CGSize(width: CGFloat(width), height: CGFloat(height)))
        }
        else {
