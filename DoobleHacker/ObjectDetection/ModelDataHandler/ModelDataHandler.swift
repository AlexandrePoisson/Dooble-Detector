...
struct Result {
    ...
    let dooblePic: String // added by APN to store the class which appears twice
}
...
class ModelDataHandler: NSObject {
    ...
    func runModel(onFrame pixelBuffer: CVPixelBuffer) -> Result? {
        ...
        // Find the picture which appears multiple time
        var all_classes = [String]()
        for res in resultArray {
            all_classes.append(res.className)
        }
        let duplicates = Array(Set(all_classes.filter({ (i: String) in all_classes.filter({ $0 == i }).count > 1})))
        ...
        let result = Result(inferenceTime: interval, inferences: resultArray, dooblePic: dooble_pic)
        return result
    }
